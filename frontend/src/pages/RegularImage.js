...
//     try {
//       const response = await fetch('http://localhost:5000/process_reg', {
//         method: 'POST',
//         body: formData,
//       });
//       if (!response.ok) {
...
//         <h2 className="text-2xl mb-4">Upload any medical image to diagnose</h2>
...
const RegPred = () => {
  ...
  const handleProcessImage = async () => {
    ...
    try {
      ...
      setLlmRes('Error processing image. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-gradient-to-br from-green-200 to-blue-900 min-h-screen flex items-center justify-center bg-gray-100">
      <div className="min-w-md w-full bg-white p-8 rounded-lg shadow-lg">
        ...
        {loading ? (
          ...
          <button onClick={handleProcessImage} disabled={!file} className="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 mb-4">
            Process Image
          </button>
        )}
        {llmRes && (
